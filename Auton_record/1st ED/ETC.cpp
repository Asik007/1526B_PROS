...
//These #defines tell which ports the motors are plugged into
...
//state is the state of intake
int state = 0;
// Lift_rvr just reverses ( L&R -> R&L) the lift if needed
bool Lift_rvr = true;
//The variable that controls how fast the intake is
float intake_CE = 1;
...
//This is defining the solenoid control ports
...
void lift_control(){
    ...
    //This allows for the lift to be raised and held at a certain point

    //The control scheme is just X for up and Y for down and when release hold the position
...
//The function I call to allow us to use the solenoid functions
...
//Here I set it up so that 3 direction let one jump between modes ( down -> stop, right -> forward) of the intake
// The 4th button then cycles through all the modes
void intake_control(){
